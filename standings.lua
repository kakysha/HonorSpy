...
function HonorSpyStandings:Refresh()
	if (T:IsRegistered("HonorSpyStandings") and T:IsAttached("HonorSpyStandings")) then
...
function HonorSpyStandings:BuildStandingsTable()
  ...
  for playerName, player in pairs(HonorSpy.db.realm.hs.currentStandings) do
    table.insert(t, {playerName, player.class, player.thisWeekHonor, player.lastWeekHonor, player.standing, player.RP, player.rank, player.is_outdated, player.last_checked})
  end
  local sort_column = 6; -- RP
  if (HonorSpy.db.realm.hs.sort == "ThisWeekHonor") then sort_column = 3; end
  table.sort(t, function(a,b) -- sort by rank
...
function HonorSpyStandings:OnTooltipUpdate()
  ...
  for i = 1, table.getn(t) do
    local name, class, thisWeekHonor, lastWeekHonor, standing, RP, rank, is_outdated, last_checked = unpack(t[i])
    
    local tooltip_outdated = ""; 
    if (is_outdated == false) then
    	is_outdated = class;
    else
    	is_outdated = "nil";
    	tooltip_outdated = " - prev week data";
    end;
    
    local last_seen, last_seen_human = (utctime() - last_checked), ""
    ...
    cat:AddLine(
      ...
      "text2", C:Colorize(BC:GetHexColor(is_outdated), string.format("%d", thisWeekHonor)),
      "text3", C:Colorize(BC:GetHexColor(is_outdated), string.format("%d", lastWeekHonor)),
      "text4", C:Colorize(BC:GetHexColor(is_outdated), string.format("%d", standing)),
      "text5", C:Colorize(BC:GetHexColor(is_outdated), string.format("%d", RP)),
      "text6", C:Colorize(BC:GetHexColor(is_outdated), string.format("%d", rank)),
      "func", function() end,
      "onEnterFunc", function()
      	GameTooltip:SetOwner(this, "ANCHOR_CURSOR")
		GameTooltip:AddLine(last_seen_human..tooltip_outdated, 1, 1, 1)
		GameTooltip:Show()
	end,
